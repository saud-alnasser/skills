...
**This repository's own tooling** — package manager, test runner, typechecker, linter, build, deploy — lives in `.claude/tools/*.md`, in the same format. `/configure` writes and maintains those files from what it discovers in the repository; when they go out of date, that is what to re-run.

Read `.claude/tools/` before running anything repo-specific.
