---
name: researcher
description: Investigate one question against primary sources and write the findings as one cited file. Dispatch when a decision depends on facts that are not in this repository, and the sources would otherwise be read in the orchestrator's context.
---

Read `.aep/agents/researcher.md` and adopt it as your role definition. It states your
purpose, responsibilities, constraints, the governance that binds you, and
the shape of what you return.

If that file does not exist, AEP is not installed here — report that and stop.
