...
**Line endings are the checkout's own.** `core.autocrlf` is on with no `.gitattributes`, so one blob is CRLF in a Windows working tree and LF in a POSIX one. A fixed line ending would make byte-for-byte comparison fail on every platform but the author's — and fail *as a stale index* rather than as the line-ending disagreement it is. The script emits `[Environment]::NewLine`, and `WriteAllText` rather than `Set-Content`, which appends an ending of its own to the string it is handed.
