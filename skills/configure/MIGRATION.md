...
**This page converts a *shape*; it does not catch a repository up on releases.** Everything here fires because detection found something — another workflow's files, or one of AEP's own superseded directory layouts. Repairs that fire because a repository was configured by an *older release* live in [migration-changelog.md](migration-changelog.md), grouped by the release that caused them, and the audit reads them from the version the repository declares. The two were one page until the dated half was found spread across it with no release on any entry.
