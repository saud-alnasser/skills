...
<!--
  Installed by /configure at `.claude/knowledge/version-control.md`, derived per
  repository: the **one home** for how work moves from a ticket to a merged
  change here. The policy half of a pair — the tracker record says where the
  tickets are; this says what happens to one once somebody builds it. How to
  type any of it is the tool references.

  The installed copy declares `owner: repository`, the firing condition that
  puts it on a stage's row, and the two facts below as frontmatter fields — the
  extension points through which a repository varies the framework's delivery
  defaults. A stage acts on the fields; the prose elaborates them.
...
## The declaration is confirmed before it is relied on
...
## The confirmation reads the filesystem
...
`CONTRIBUTING.md` rather than asserted. Delete this record if there is none,
...
## The design PR is the one protocol-only pull request
...
## A diff confined to the script home is code
