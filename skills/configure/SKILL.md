...
- **Sweep every committed file under `.claude/`, and classify each one.** Enumerate the committed set from version control rather than from a walk of the working tree — `git ls-files .claude` on git, and `.claude/tools/` carries the invocation — because a walk sees per-clone files that are not this repository's knowledge and misses nothing in exchange. Classify each file by **category**, which is the directory step 4's layout names it under — **read the categories off that layout, never off a list restated here**, so a category a later release adds is swept the release it appears rather than the release somebody remembers to update two lists — and by the `owner` its frontmatter declares. Then check it against that category's contract: a framework-owned file by the comparison below; the derived guides and the tool references against what their own templates declare; `contexts/`, `decisions/`, `designs/`, and `evidence/` against the format each of their policies ships; `tickets/` against the ticket format `.claude/policies/tickets.md` declares, in the form `.claude/policies/tracker.md` selects — and a spec or generated index filed beside tickets is held to its own format, owner included, so the directory does not decide the contract — **no owner declaration is required of a ticket**, because its format declares none and a swept file is held to its own category's contract rather than to a neighbouring one.
- **Quote the enumeration and the counts; a sentence claiming coverage is not coverage.** Print the number of files enumerated, the number classified per category, and every path in neither — **the swept count equals the enumerated count, and any difference is the finding**. Coverage stated in prose is the judgement this sweep exists to replace: it reads identically whether the sweep reached the whole tree or the four categories it used to reach.
- **A committed file fitting no category is a finding, reported with its path** — a second loose file beside the router, a directory the layout does not name, a knowledge file nothing routes to. It is never filed under the nearest neighbouring category, because that is how a file nobody planned becomes permanent and stops being reported.
- **A governed file declaring no owner is a finding.** Absence is not read as a default: a file that is repository-owned by design and one written before the declaration existed are otherwise indistinguishable forever, and no later run can tell them apart either. What is swept without one is named here rather than left to a run's judgement — **ticket issue files**, whose fields are their own format's, and the committed machinery that carries no frontmatter to declare in: the ignore file, the scripts under `.claude/scripts/`, and the harness's `settings.json` below. `CLAUDE.md` sits at the repository root, outside the enumerated set entirely — the harness loads it by name.
- **Exempt exactly the per-clone set `.claude/.gitignore` defines — `/position/`, `/worktrees/`, `settings.local.json` — and nothing else.** They are outside the committed set by construction, so a per-clone path reported as a finding means the enumeration read the working tree, and the sweep is what is wrong rather than the repository. `.claude/settings.json` is committed and is the harness's: swept for presence and for the base ref above, never for an owner declaration it would be wrong to carry.
- **Re-check every installed instruction file against its owner's contract** — every file the sweep classified, rather than a named list of categories that leaves the rest of the tree unexamined. **A file declaring `owner: framework` is compared against the release's template exactly, and the comparison is computed, never eyeballed** — for the protocol file, after setting aside the extension point its own opening paragraph names — the Deviations entries, the repository's own, surviving both reinstall and upgrade: materialise both sides, normalise line endings, and quote the output of a real comparison command (`fc`, `diff`, or a hash pair) — then any remaining difference is a **defect to reinstall, never drift to heal**: verbatim replacement, no healing language, no ask, because framework law carries no local corrections to preserve. (A derived comparison *script* is not the shape here, deliberately: the templates live in the plugin, whose root the harness exposes to skill content and hooks but not to a stage's shell — so the stage materialises and compares at use instead.) **A repository-owned file** — the derived guides, the tool references — is where the ask-branch below still holds.
- **The `version` stamp a framework-owned file carries routes attention and settles nothing: the comparison above runs whatever the stamp says.** It is provenance — the release that last changed that file — so a stale-looking stamp is a reason to look sooner, and a current-looking one licenses skipping nothing.
