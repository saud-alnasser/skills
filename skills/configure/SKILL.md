...
**`.claude/protocol.md`**, from [protocol.template.md](protocol.template.md), copied as-is — which stamps its `aep-version` field correctly by construction, because the template ships with the release that ships it. The router rather than a rule — the Marker, the drift reads, the verification report, and the table saying which guides each stage reads — so it is reached by pointer and a question turn does not pay for it.
...
- **Re-check the installed copies against the templates that wrote them** — the rules, the modes, every policy installed by copy, and the protocol file. A copied file has no repository-side source to check it against, and no other bullet compares a copied file against its template — so this drift is invisible to every other check in this list. Finding a difference is this bullet's whole job; how the comparison is made and what a difference means are stated below, because disposing of one is a separate act from finding it.

That bullet's comparison — and any other read of an installed file against what AEP ships — is made one way. **Compare content, never bytes** — strip carriage returns from both sides before comparing, and ignore trailing whitespace. The plugin's checkout and the configured repository can disagree about line endings for reasons that have nothing to do with either file: a repository that pins `eol=lf` and a plugin checked out on a Windows default differ on every line of every file while being identical, and that is the ordinary case on Windows rather than an unusual one.

**Drift reported across every file at once is a fault in the comparison until proven otherwise**, and it is checked before it is acted on. This one is singled out because its correction is the only destructive action in this pass: re-deriving an installed file from its template discards whatever that repository put there, and the derived tool references are exactly where a repository keeps corrections it verified against its own tooling — the upward half of [TOOLS.md](TOOLS.md)'s refresh path exists because those corrections can be *ahead of* what AEP ships. An audit that re-derives them has not repaired the repository; it has reverted it, and nothing later in this pass will notice.

So a genuine difference is **reported with the file named**, and what happens next depends on which kind it is. A file nobody touched on purpose has drifted and is re-derived. A file carrying a deliberate local correction is left alone and the difference is stated, because that is a finding for a human rather than a repair — and the two are not distinguishable by looking at the diff, which is why this asks rather than decides.
