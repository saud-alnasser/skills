...
- **Vendored skills are altered, not rewritten**, so the copied text stays recognisable in the file and its attribution rides with it. That obligation reaches vendored text and nothing else — a skill that borrowed only a shape carries none, and `.claude/rules/skills.md` holds the test rather than restating it here (ADR 0066).
