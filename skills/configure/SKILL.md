...
Several checks above compare an installed file against what AEP ships. **Compare content, never bytes** — strip carriage returns from both sides before comparing, and ignore trailing whitespace. The plugin's checkout and the configured repository can disagree about line endings for reasons that have nothing to do with either file: a repository that pins `eol=lf` and a plugin checked out on a Windows default differ on every line of every file while being identical, and that is the ordinary case on Windows rather than an unusual one.
