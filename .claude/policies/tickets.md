...
So: **no implementation diary.** Not what you tried, not what went wrong on the way, not a running log of the session. Detailed engineering belongs in a spec, which the ticket **references** — never pastes. Where a spec lives is `.claude/policies/tracker.md`'s.
