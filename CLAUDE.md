...
This repository builds the **AI Engineering Protocol (AEP)** — a Claude Code skill framework where understanding of a repository compounds over time. `skills/` is what ships, `.claude/` is what this repository runs on, and `specs.md` is the canonical specification.
